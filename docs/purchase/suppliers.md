...
Relationships: `purchaseOrders()` hasMany, `supplierReturns()` hasMany ReturnOrder (scoped to `type = supplier_return`), `products()` belongsToMany via `product_supplier` pivot (`unit_cost`, `supplier_sku`)
...
Pages: `ListSuppliers`, `CreateSupplier`, `EditSupplier`
