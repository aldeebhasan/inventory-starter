...
**Referenced by:** Return Orders — a customer return can reference a SO as its `originalOrder`.

**Do not:** Call `$product->deductStock()` directly for a sale. All stock deductions from selling must go through a SaleOrder → Ship workflow. Do not skip the reservation step (Confirm) — shipping without prior confirmation bypasses the stock hold and risks overselling.
...
- `SaleOrder` — `HasFactory`, `SoftDeletes`; casts: `status => SaleOrderStatus`, `ordered_at`, `shipped_at` as datetime; relationships: `customer()`, `location()`, `items()`, `createdBy()`, `returnOrders()` morphMany
...
- `ListSaleOrders`, `CreateSaleOrder`, `ViewSaleOrder`, `EditSaleOrder` (Draft only)
...
**PickAction** — `Confirmed → Picked` (no inventory call — preparation step)
```php
Action::make('pick')
